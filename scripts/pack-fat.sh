...
install -m 755 "$ROOT/scripts/slot-lib.sh" "$BUNDLE/opt/cozmars/slot-lib.sh"
mkdir -p "$BUNDLE/usr/local/lib/cozmars" "$BUNDLE/usr/local/bin"
install -m 644 "$ROOT/scripts/slot-lib.sh" "$BUNDLE/usr/local/lib/cozmars/slot-lib.sh"
install -m 755 "$ROOT/scripts/cozmars-boot-guard.sh" "$BUNDLE/usr/local/bin/cozmars-boot-guard"
